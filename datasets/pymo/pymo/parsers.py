...
class BVHWriter():
    '''A class to write MocapData to BVH file format'''
    
    def __init__(self):
        pass
    
    def write(self, data, filename):
        '''Write MocapData to BVH file'''
        with open(filename, 'w') as f:  
            self._write_hierarchy(f, data)
            self._write_motion(f, data)
    
    def _write_hierarchy(self, f, data):
        '''Write the HIERARCHY section'''
        f.write("HIERARCHY\n")
        root_name = data.root_name
        self._write_joint(f, data, root_name, 0)
    
    def _write_joint(self, f, data, joint_name, level):
        '''Recursively write joint hierarchy'''
        joint = data.skeleton[joint_name]
        indent = "  " * level
        
        if joint['parent'] is None:
            f.write(f"{indent}ROOT {joint_name}\n")
        elif joint_name.endswith('_Nub'):
        ...
        if not joint_name.endswith('_Nub'):
    ...
    def _write_motion(self, f, data):
        ...
        f.write("MOTION\n")
        frame_count = len(data.values)
        f.write(f"Frames: {frame_count}\n")
        f.write(f"Frame Time: {data.framerate:.6f}\n")
        
        for idx, row in data.values.iterrows():
            values = row.values
