...
class RootTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, method):
        """
        Accepted methods:
            abdolute_translation_deltas (typo retained for backward compatibility)
            absolute_translation_deltas (common spelling — accepted as alias)
            pos_rot_deltas
        """
        # Accept both spellings to be robust to callers using 'absolute'
        if method == 'absolute_translation_deltas':
            method = 'abdolute_translation_deltas'
        if method not in ('abdolute_translation_deltas', 'pos_rot_deltas'):
            raise ValueError(f"Unsupported RootTransformer method: {method}")
        self.method = method
